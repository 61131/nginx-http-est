...
* **syntax:** `est_ca_default_days <days>`
* **default:** `30`
* **context:** `location`

Configures the default validity interval - in days - for newly signed certificates.

This directive specifies the length of time, in days following creation date, that certificates generated by the certificate authority will be valid. This interval may be overridden by validity dates supplied within the client certificate signing request (CSR) if the directive est_ca_preserve_dates is set to true.

### est_ca_preserve_dates

* **syntax:** `est_ca_preserve_dates on|off`
* **default:** `off`
* **context:** `location`

Configures whether to preserve dates in certificate signing requests (CSRs) when generating certificates.

This directive specifies with certificate validity start and end dates in certificate signing requests (CSRs) will be incorporated into signed certificates issued by the EST server. If disabled, the EST server will generated certificates that are only valid for the interval from the date of creation - in days - specified in the est_ca_default_days directive.
...
Configures the private key to be used in association with the certificate authority trust anchor.

This directive specifies the private key - in PEM format - to be employed in association with the certificate authority trust anchor for signing operations. 
